...
原文链接：https://blog.csdn.net/Aprilyang42/article/details/114639444
...
-- 事务开启(transaction)
START TRANSACTION 或  Begin-- 标记一个事务的开启，之后的sql操作都是属于一个事务
...
**start transaction       和      set @@autocommit =0 **



| **方面**             | **START TRANSACTION**                         | **SET @@autocommit=0**                   |
| -------------------- | --------------------------------------------- | ---------------------------------------- |
| **作用范围**         | **只针对当前事务生效**                        | 改变当前会话的自动提交模式，**持续生效** |
| **事务控制**         | 必须手动执行 `COMMIT` 或 `ROLLBACK` 结束事务  | 每条语句都在事务中执行，需手动提交或回滚 |
| **隐式提交的处理**   | 未提交时不影响新语句（需再次用 `START` 开启） | 关闭自动提交后，每条语句都受事务保护     |
| **适用场景**         | 用于处理特定逻辑时开启的事务                  | 用于需要在整个会话中控制事务行为的场景   |
| **自动提交默认状态** | 自动提交模式不受影响                          | 自动提交模式被关闭                       |
| **使用体验**         | 更灵活，只在需要时开启                        | 简化了多次事务控制，但需记住提交         |





### 1. **为什么开启事务之后没有 `COMMIT`，但表里数据会有变化？**

当你在开启事务后执行了插入、更新、删除等操作时，数据库会立即在表中**临时应用这些更改**，即这些更改会对**当前事务中的会话可见**。这是因为：

- **事务的本质**是确保一组操作要么全部执行成功（`COMMIT`），要么全部撤销（`ROLLBACK`），但在**事务完成之前，这些变更是临时的**。
- 在事务期间，**其他会话**无法看到你的操作结果（取决于隔离级别），但**当前会话**可以看到这些未提交的更改。

因此，虽然看似数据发生了变化，但如果你不执行 `COMMIT`，这些变更**不会持久保存**到数据库里。
...
\1. 假设学号是表中的唯一主键，那由学号就可以确定姓名和年龄了，但是却不能确定课程名称（语文或数学）和成绩。
...
\3. 成绩表 - 学号和课程名称做联合主键（**要想确认成绩，必须有学号和课程名称**）
...
- 第三范式（3 NF）：在任一主键都可以确定所有非主键字段值的情况下，不能存在某非主键字段 A 可以确定某非主键字段 B。
...
public class Jdbc_Druid_Utils {
    ...
    static {
        try {
            InputStream in = Jdbc_Druid_Utils.class.getClassLoader().getResourceAsStream("druid.properties");
            //定义配置文件
